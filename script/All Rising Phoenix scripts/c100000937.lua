...
function s.initial_effect(c)
	...
	e5:SetCondition(s.descontd)
	e5:SetTarget(s.destgtd)
	e5:SetOperation(s.desoptd)
	c:RegisterEffect(e5)
end
function s.descontd(e,tp,eg,ep,ev,re,r,rp)
	local d=Duel.GetAttackTarget()
	return e:GetHandler()==Duel.GetAttacker() and d and d:IsFacedown() and d:IsAbleToDeck()
end
function s.destgtd(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetOperationInfo(0,CATEGORY_TODECK,Duel.GetAttackTarget(),1,0,0)
end
function s.desoptd(e,tp,eg,ep,ev,re,r,rp)
	local d=Duel.GetAttackTarget()
...
function s.spcon(e)
	...
	return Duel.IsExistingMatchingCard(s.filterd,tp,0,LOCATION_MZONE,1,nil)
end
function s.filter(c)
