...
function s.cfilter(c)
	return c:IsFaceup() and c:IsRank(1) and c:IsType(TYPE_XYZ)
end
function s.spfilter(c,e,tp)
	return c:IsRace(RACE_PYRO) and c:IsAttribute(ATTRIBUTE_WATER) and c:IsLevelBelow(5) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
