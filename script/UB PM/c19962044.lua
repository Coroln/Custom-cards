...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	s.announce_filter={0x7CC,OPCODE_ISSETCARD,code,OPCODE_ISCODE,OPCODE_NOT,OPCODE_AND,TYPE_MONSTER,OPCODE_ISTYPE,OPCODE_AND}
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0
		or not Duel.IsPlayerCanSpecialSummonMonster(tp,ac,0,TYPE_MONSTER|TYPE_NORMAL,0,0,Duel.GetCardRaceFromCode(ac),Duel.GetCardLevelFromCode(ac),ATTRIBUTE_LIGHT) then return end
	c:AddMonsterAttribute(TYPE_NORMAL|TYPE_TRAP,ATTRIBUTE_LIGHT,Duel.GetCardRaceFromCode(ac),Duel.GetCardLevelFromCode(ac),1000,1000)
