...
function s.initial_effect(c)
    ...
    --Must be either Link Summoned or Special Summoned by alternate procedure
...
function s.splimit(e,se,sp,st)
	return (st&SUMMON_TYPE_LINK)==SUMMON_TYPE_LINK or e:GetHandler():GetLocation()~=LOCATION_EXTRA 
end
--Special summon procedure
...
function s.spop2(e,tp,eg,ep,ev,re,r,rp)
	...
	local c=e:GetHandler()
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local tc=Duel.SelectMatchingCard(tp,s.spfilter2,tp,LOCATION_DECK+LOCATION_HAND,0,1,1,nil,e,tp):GetFirst()
	if tc and Duel.SpecialSummonStep(tc,0,tp,tp,false,false,POS_FACEUP) then
	end
	Duel.SpecialSummonComplete()
end
--change a monster into "Majestic Dragon"
function s.tunfilter(c)
	return c:IsFaceup() and c:IsType(TYPE_TUNER) and not c:IsCode(21159309)
end
