...
function s.initial_effect(c)
    -- Fusion summon on summon
    local e1=Effect.CreateEffect(c)
    e1:SetDescription(aux.Stringid(id,0))
    e1:SetCategory(CATEGORY_DICE+CATEGORY_SPECIAL_SUMMON+CATEGORY_FUSION_SUMMON)
...
function s.fusop(e,tp,eg,ep,ev,re,r,rp)
	...
	local sum=d1+d2
	local params={aux.FilterBoolFunction(Card.IsLevel,sum)}
	-- Now just call the actual Fusion Summon operation
