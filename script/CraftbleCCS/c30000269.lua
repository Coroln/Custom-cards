...
function s.filter(c)
	return c:GetLevel()>0 and c:IsSetCard(0x18) and c:IsAbleToGrave()
end
	...
	if #g>0 then
		...
		if Duel.SendtoGrave(g,REASON_EFFECT)~=0 and sg:IsLocation(LOCATION_GRAVE) then
			...
			if tc:IsFaceup() and tc:IsRelateToEffect(e) then
				tc:AddCounter(0x1019,sg:GetLevel())
			end
		end
	end
end
function s.thfilter(c)
	return c:IsSetCard(0x18) and c:IsMonster() and c:IsAbleToHand()
end
