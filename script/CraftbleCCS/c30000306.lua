...
function s.cfilter(c,tp)
	return c:IsMonster() and c:IsSetCard(SET_FORBIDDEN_ONE) and c:IsPreviousControler(tp)
end
...
function s.drop(e,tp,eg,ep,ev,re,r,rp)
	...
	local ct=Duel.GetMatchingGroupCount(s.cfilter,c:GetControler(),LOCATION_HAND,0,nil)
...
function s.discon(e,tp,eg,ep,ev,re,r,rp)
	...
	if not tg or not tg:IsExists(s.cfilter,1,nil,tp) then return false end
	return re:IsHasType(EFFECT_TYPE_ACTIVATE) and Duel.IsChainNegatable(ev)
end
function s.disop(e,tp,eg,ep,ev,re,r,rp)
	local rc=re:GetHandler()
	if not (re:GetHandlerPlayer()==1-tp) then return end
	if not re:IsHasProperty(EFFECT_FLAG_CARD_TARGET) then return false end
