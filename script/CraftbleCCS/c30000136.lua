...
function s.spfilter(c)
	return c:IsCode(CARD_UMI)
end
function s.spcon(e,c)
	if c==nil then return true end
	local tp=e:GetHandlerPlayer()
	local rg=Duel.GetMatchingGroup(s.spfilter,tp,LOCATION_HAND+LOCATION_FIELD,0,nil)
...
function s.filter(c)
	return (c:IsCode(CARD_UMI) or c:IsCode(43889633)) and c:IsAbleToDeck()
end
