...
function s.initial_effect(c)
    ...
    e1:SetCategory(CATEGORY_DRAW+CATEGORY_SPECIAL_SUMMON+CATEGORY_DESTROY+CATEGORY_TOHAND+CATEGORY_DAMAGE)
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
    ...
    -- Wir legen ein Lua-table an, um ct und op_sel zu speichern
    local data={}
    data.ct = ct       -- Anzahl gesendeter Monster
    data.op_sel = 0    -- welche Effektoption der Spieler wählt (Standard: 0 = keine)

    -- Nur wenn genau 3 Monster gesendet wurden, fragen wir die Bonuseffekte ab
    if ct==3 then
        -- Zähle jetzt alle Drachen-Normalen im Friedhof
        ...
        if gc>=10 and Duel.IsExistingMatchingCard(s.Searchfilter,tp,LOCATION_DECK,0,1,nil) then
        ...
        if #op_table>0 then
            ...
            data.op_sel = op_val[sel+1] -- Effektwahl speichern
        end
    end
    
    -- Speichere das gesamte table in der LabelObject
    e:SetLabelObject(data)
end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
    ...
    local ct = data.ct
    local op_sel = data.op_sel
    
    -- Wenn nicht genau 3 gesendet oder keine Option gewählt
    if ct~=3 or op_sel==0 then return end
    
    -- Nun je nach op_sel den Effekt ausführen
    if op_sel==1 then
        -- 1) Ziehe 1 Karte
    ...
    elseif op_sel==3 then
        -- 3) Zerstöre 1 gegnerische Karte
    ...
    elseif op_sel==5 then
        ...
        Duel.Win(tp,0x60) -- oder Duel.Win(tp,0)
    end
end
