--Finsterer Empfänger Undercroft Sleeper
local s,id=GetID()
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Special summon itself (GY) and 1 level 4 or lower Warrior/Spellcaster monster (hand)
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(id,0))
...
function s.filter(c,e,tp)
	return c:IsLevelBelow(4) and (c:IsRace(RACE_WARRIOR) or c:IsRace(RACE_SPELLCASTER)) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
