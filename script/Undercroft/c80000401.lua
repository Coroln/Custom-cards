...
function s.cfilter(c,tp)
	return c:IsAttribute(ATTRIBUTE_DARK) and c:IsFaceup() and c:IsSummonPlayer(tp)
end
...
function s.sumfilter(c)
	return c:IsSetCard(0x5344) and c:IsSummonable(true,nil)
end
...
function s.costfilter(c)
	return c:IsSetCard(0x5344) and c:IsType(TYPE_MONSTER) and c:IsAbleToGraveAsCost()
end
...
function s.thfilter(c)
	return c:IsSetCard(0x5344) and c:IsType(TYPE_SPELL+TYPE_TRAP) and c:IsAbleToHand()
end
