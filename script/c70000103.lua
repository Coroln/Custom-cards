...
--Monster filter
function s.filter(c)
	return c:IsRace(RACE_HIGHDRAGON) and c:IsAttribute(ATTRIBUTE_FIRE)
end
--Trap filter
...
function s.rfilter(c)
	return (c:IsRace(RACE_HIGHDRAGON) or c:IsAttribute(ATTRIBUTE_FIRE)) and c:IsAbleToRemoveAsCost() 
...
function s.damcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.cfilter,tp,LOCATION_REMOVED,0,1,nil) and Duel.GetCurrentPhase()==PHASE_MAIN1 end
