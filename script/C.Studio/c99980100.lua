...
function s.initial_effect(c)
  --Xyz Summon
  Xyz.AddProcedure(c,aux.FilterBoolFunctionEx(Card.IsSetCard,0x998),3,2)
  c:EnableReviveLimit()
  --(1) Special Summon
...
function s.spfilter(c,e,tp)
  return c:IsType(TYPE_MONSTER) and c:IsSetCard(0x998) and c:IsLevelBelow(4) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function s.xyzfilter(c,e,tp)
  return c:IsSetCard(0x2998) and c:GetRank()==4  and e:GetHandler():IsCanBeXyzMaterial(c,tp)
