...
function s.initial_effect(c)
  ...
  e1:SetProperty(EFFECT_FLAG_CHAIN_UNIQUE+EFFECT_FLAG_DELAY+EFFECT_FLAG_DAMAGE_STEP+EFFECT_FLAG_DAMAGE_CAL)
  ...
  e2:SetProperty(EFFECT_FLAG_CHAIN_UNIQUE+EFFECT_FLAG_CARD_TARGET+EFFECT_FLAG_DELAY+EFFECT_FLAG_DAMAGE_STEP+EFFECT_FLAG_DAMAGE_CAL)
  e2:SetRange(LOCATION_FZONE)
  e2:SetCountLimit(1)
  e2:SetCost(s.xyzcost2)
...
function s.xyzconfilter(c,e,tp)
  ...
  return c:IsSetCard(0x998) and c:GetSummonPlayer()==tp and not c:IsType(TYPE_XYZ) and ck>=PHASE_BATTLE_START and ck<=PHASE_BATTLE
end
function s.mfilter(c)
  return c:IsFaceup() and c:IsSetCard(0x998) and not c:IsType(TYPE_TOKEN)
end
function s.xyzfilter1(c,mg)
  return c:IsSetCard(0x998) and c:IsXyzSummonable(mg)
end
...
--(2)
function s.xyzcost2(e,tp,eg,ep,ev,re,r,rp,chk)
  e:SetLabel(1)
  if chk==0 then return true end
end
function s.xyzcostfilter(c)
  return c:IsFaceup() and c:GetType()==TYPE_SPELL+TYPE_CONTINUOUS and aux.IsCodeListed(c,99980030) and c:IsAbleToGraveAsCost()
end
function s.xyzfilter2(c,e,tp)
  return c:IsSetCard(0x998) and c:GetRank()==4 and c:GetSummonPlayer()==tp and c:IsLocation(LOCATION_MZONE)
...
function s.xyzfilter3(c,e,tp,mc,code)
  ...
  return mc:IsType(TYPE_XYZ,c,SUMMON_TYPE_XYZ,tp) and c:IsSetCard(0x998) and aux.IsCodeListed(c,code) and c:GetRank()==5
