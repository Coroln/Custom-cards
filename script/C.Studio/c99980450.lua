...
function s.anfilter(c)
  return c:IsType(TYPE_MONSTER) and not c:IsSetCard(0x1998) and c:GetLevel()>0
end
...
function s.anop(e,tp,eg,ep,ev,re,r,rp)
  ...
  for tc in aux.Next(g) do
    ...
    tc:RegisterFlagEffect(0,RESET_EVENT+0x1fe0000+RESET_PHASE+PHASE_END,EFFECT_FLAG_CLIENT_HINT,1,0,aux.Stringid(id,0))
  end
end
--(2) Destroy
function s.desfilter(c)
  return c:IsFaceup() and c:IsType(TYPE_MONSTER) and c:IsSetCard(0x1998)
end
