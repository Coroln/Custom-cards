...
function s.xyzfilter1(c,e,tp)
  ...
  return pg:GetCount()<=1 and rk>0 and c:IsFaceup() and c:IsSetCard(0x2998) and (rk>0 or c:IsStatus(STATUS_NO_LEVEL)) and Duel.GetLocationCountFromEx(tp,tp,c)>0 
...
function s.xyzfilter2(c,e,tp,mc,rk,pg)
  ...
  return c:IsType(TYPE_XYZ) and mc:IsType(TYPE_XYZ,c,SUMMON_TYPE_XYZ,tp) and c:IsRank(rk) and c:IsSetCard(0x2998) and mc:IsCanBeXyzMaterial(c,tp)
