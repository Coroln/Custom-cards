...
--(1) Draw
function c99980280.drtg(e,tp,eg,ep,ev,re,r,rp,chk)
  if chkc then
...
function c99980280.drop(e,tp,eg,ep,ev,re,r,rp)
  ...
  if tc:IsSetCard(0x998) then
    ...
    and Duel.SelectYesNo(tp,aux.Stringid(99980280,1)) then
      Duel.Hint(HINT_OPSELECTED,1-tp,aux.Stringid(99980280,2))
      Duel.SpecialSummon(tc,0,tp,tp,false,false,POS_FACEUP)
    elseif b2 and tc:CheckActivateEffect(false,true,false)~=nil and Duel.SelectYesNo(tp,aux.Stringid(99980280,3)) then
      Duel.Hint(HINT_OPSELECTED,1-tp,aux.Stringid(99980280,4))
    ...
    elseif not (b1 or b2) and tc:IsAbleToDeck() and Duel.IsPlayerCanDraw(tp,1) and Duel.SelectYesNo(tp,aux.Stringid(99980280,5)) then
      Duel.Hint(HINT_OPSELECTED,1-tp,aux.Stringid(99980280,6))
