...
function s.initial_effect(c)
  c:EnableReviveLimit()
  --Link Summon
  Link.AddProcedure(c,aux.FilterBoolFunctionEx(Card.IsSetCard,0x998),2,2)
...
function s.tdcon(e,tp,eg,ep,ev,re,r,rp)
  ...
  return c:IsLocation(LOCATION_GRAVE) and r==REASON_LINK and c:GetReasonCard():IsSetCard(0x998)
end
...
function s.actfilter(c,tp)
  return c and c:IsFaceup() and c:IsSetCard(0x998) and c:IsType(TYPE_MONSTER) and (c:IsType(TYPE_XYZ) or c:IsType(TYPE_LINK)) and c:IsControler(tp)
end
