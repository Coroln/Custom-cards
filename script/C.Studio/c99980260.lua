...
function s.atkfilter(c)
  return c:IsSetCard(0x1998) and c:IsType(TYPE_MONSTER)
end
...
function s.reccon(e,tp,eg,ep,ev,re,r,rp)
  ...
  if des:IsReason(REASON_BATTLE) then
    ...
    return rc and rc:IsSetCard(0x998) and rc:IsType(TYPE_MONSTER) and rc:IsType(TYPE_XYZ) 
    and rc:IsControler(tp) and rc:IsRelateToBattle() and e:GetHandler():GetFlagEffect(id)>0
  end
  return false
end
function s.reccostfilter(c)
  return c:IsSetCard(0x1998) and c:IsType(TYPE_MONSTER) and c:IsAbleToGraveAsCost()
end
