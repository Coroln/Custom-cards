...
function s.initial_effect(c)
	...
	e3:SetTarget(s.e3thtg)
	e3:SetOperation(s.e3thop)
	c:RegisterEffect(e3)
end
...
function s.e1Target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.IsExistingTarget(aux.AND(Card.IsAbleToGrave, Card.IsFaceup),tp,0,LOCATION_MZONE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g=Duel.SelectTarget(tp,aux.AND(Card.IsAbleToGrave, Card.IsFaceup),tp,0,LOCATION_MZONE,1,1,nil)
...
function s.e3filter(c)
	return c:IsRitualSpell() and c:IsAbleToHand() and c:IsSetCard(0xCF)
end
function s.e3thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.e3filter,tp,LOCATION_GRAVE,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_GRAVE)
end
function s.e3thop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectMatchingCard(tp,s.e3filter,tp,LOCATION_GRAVE,0,1,1,nil)
	if #g>0 then
